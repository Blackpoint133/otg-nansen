...
Task 026 adds a pure deterministic hourly backfill planner informed by Task 025's observed lower-bound omission. Each unit separates desired `coverage_start/coverage_end` from Nansen `request_start/request_end`; requests start one hour before coverage, and units contain at most 167 desired hourly starts. The canonical target from 2025-04-25T00:00:00Z through 2026-09-20T23:00:00Z produces 12,336 desired starts across 74 exact-window units. Completion is determined from a matching successful ingestion audit with non-null sanitized `source_warnings`; retained row counts and the stream high-water checkpoint are not completion evidence. The default planner command is a dry run and makes no API calls or database writes. Task 026 did not execute any backfill.

The current [Nansen credits documentation](https://docs.nansen.ai/getting-started/credits) lists TGM Flows at one credit per call for both Free and Pro. A 74-credit figure is only an estimate assuming one API request per planned unit. The account plan and balance were not queried or verified.

Task 027's guarded staging runner requires `--execute` and `NANSEN_RUN_LIVE_BACKFILL=1` together. For the first three canonical historical units, it used one request per unit, no retries, and one final page per request. Each unit returned 167 normalized hourly records and passed the exact warning fingerprint and structural guard. The corresponding sanitized audit summaries contain only `NON_EXCHANGE_BREAKDOWN_UNAVAILABLE`. This validates only those three bounded request windows; no later unit was requested.

Task 028 generalized the staging runner to resume from exact-window audit progress. It selected the next ten pending units (4–13) under a per-invocation cap of 20 units and 20 calls, using one request per unit and no retries. Each returned 167 normalized rows, passed the exact warning fingerprint and structural breakdown guard, and recorded only the sanitized `NON_EXCHANGE_BREAKDOWN_UNAVAILABLE` category in its audit. This verifies only units 4–13; unit 14 is next and the remaining 61 units were not requested.

Task 029 removed Task-028-specific bounds and task-numbered status handling from the generic staging runner. A 20-unit/20-call authorization made 15 requests (units 14–28), then stopped because the validator compared UTC timestamps with database timestamps in the daylight-saving fold without normalizing both to UTC. Post-stop read-only UTC identity validation confirmed all 167 unit 28 buckets and no gap. Each request used the exact warning fingerprint policy and sanitized audit summary. Units 29–33 were not requested; production was not accessed. The UTC-normalization correction was added with an offline regression test.

Task 029R used the corrected UTC-canonical validator to complete units 29–33 with five one-call requests. Each unit had a successful exact-window audit, sanitized warning evidence, and 167/167 desired hourly identities. The Task 029 failure status remains historical; the recovery completes intended units 14–33 only. Unit 34 was not requested.

Task 030 completed units 34–45, then encountered an API error on unit 46. Task 030R made one authorized attempt for pending unit 46, which also failed; no later unit was requested. The 12 completed units retain exact-window audits with sanitized warning categories. No raw warning or response content was retained. Units 46–53 remain pending; see the Task 030 report for the verified partial state.
...
Task 031 executed units 54-73 using one request per unit through the existing Flows ingestion path. All 20 exact-window audits succeeded with non-null sanitized warning evidence, and each unit retained all 167 desired hourly starts. The approved non-exchange breakdown category was the only warning category observed. Progress is 73 complete / 1 pending / 0 ambiguous. Unit 74 remains unrequested; the checkpoint and production were unchanged.

Task 032 executed final canonical unit 74 with one TGM Flows request. The exact-window audit succeeded with one page, one API call, 145 received and normalized records, and sanitized warning evidence. All 145 desired hourly starts are retained. End-to-end staging inspection verified 12,336/12,336 canonical hourly identities, no gaps, unexpected starts, or duplicates, and contiguous one-hour spacing. No further Nansen request was made. Production was untouched.

Task 032R made no Nansen calls. It corrected terminal postconditions in the staging runner: equal-timestamp successful checkpoint owner replacement is valid after owner audit validation; selected desired recent-window additions are allowed while pre-existing identities must remain; and the final canonical unit may complete with no next pending unit. Offline tests cover these cases. Read-only verification confirms all 12,336 hourly identities and 74 successful warning-audited canonical units.
